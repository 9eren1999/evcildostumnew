 ...
 List<Veteriner> _sortedVeterinerler = [];
  ...
  Future<Position> _determinePosition() async {
    ...
    if (permission == LocationPermission.denied) {
      ...
      if (permission == LocationPermission.denied) {
        // İzin reddedildiyse hata göster
  ...
  void _sortVeterinerlerByDistance() {
    ...
    setState(() {
      _sortedVeterinerler = yozgatVeterinerleri..sort((Veteriner a, Veteriner b) {
        // İki veteriner arasındaki mesafeyi hesapla
  ...
  // doğru veteriner değişkenini al ve _veterinerCard'a geçir
  ...
  if (distance < 1000) {
    ...
    // Kilometreye çevir ve bir ondalık basamak ile formatla
  ...
  void _launchMapsUrl() async {
    final url = veteriner.mapsUrl; // Veteriner sınıfındaki URL'yi kullan
  ...
  shape: RoundedRectangleBorder(
    borderRadius: BorderRadius.circular(20.0), // Kenarları yuvarlaklaştır
  ),
  child: ListTile(
    title: Text(veteriner.isim, style: TextStyle(fontWeight: FontWeight.w600, color: Colors.grey.shade800,fontSize: 14)),
    subtitle: Text(distanceString,  style: TextStyle(fontWeight: FontWeight.w400, color: Colors.grey.shade500,fontSize: 12)),
